...
router.post('/games', [
    ...
    body('image_url', 'URL Gambar harus berupa URL yang valid').isURL(),
        body('is_active', 'Status aktif harus bernilai true atau false').isBoolean(),
        body('user_id_help', 'Teks bantuan tidak boleh terlalu panjang').optional().trim().isLength({ max: 500 })
], async (req, res) => {
    const errors = validationResult(req);
        if (!errors.isEmpty()) {
            // Kirim pesan error pertama yang ditemui agar lebih jelas
        ...
        try {
            ...
            if (err.code === '23505') { // Error untuk unique violation (slug sudah ada)
